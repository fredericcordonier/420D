...
int menu_scripts_fd =
    MENU_SCRIPTS_FD; // DOF calculator: initial focusing distance
...
menuitem_t ext_aeb_items[] = {
    ...
    MENUITEM_BULB(0, LP_WORD(L_I_MANUAL_L), &settings.eaeb_tv_min,
                  menu_scripts_apply_eaeb_tvmin),
    MENUITEM_BULB(0, LP_WORD(L_I_MANUAL_R), &settings.eaeb_tv_max,
                  menu_scripts_apply_eaeb_tvmax)};
...
menuitem_t efl_aeb_items[] = {
    ...
    MENUITEM_EAEBDIR(0, LP_WORD(L_I_DIRECTION), &settings.efl_aeb_direction,
                     NULL),
};
...
menuitem_t apt_aeb_items[] = {
    ...
    MENUITEM_EAEBDIR(0, LP_WORD(L_I_DIRECTION), &settings.apt_aeb_direction,
                     NULL),
};
...
menuitem_t interval_items[] = {
    ...
    MENUITEM_TIMEOUT(0, LP_WORD(L_I_INTERVAL), &settings.interval_time,
                     menu_scripts_update_timelapse),
    MENUITEM_COUNTER(0, LP_WORD(L_I_SHOTS), &settings.interval_shots,
                     menu_scripts_update_timelapse),
    MENUITEM_VFORMAT(0, LP_WORD(L_I_VFORMAT), &menu_scripts_vformat,
                     menu_scripts_update_timelapse),
...
menuitem_t bramp_items[] = {
    ...
    MENUITEM_EVCOMP(0, LP_WORD(L_I_RAMPING_TIME), &settings.bramp_ramp_time,
                    NULL),
    MENUITEM_EVCOMP(0, LP_WORD(L_I_RAMPING_EXP), &settings.bramp_ramp_exp,
                    NULL),
};
...
menuitem_t lexp_calc_items[] = {
    MENUITEM_BASEISO(0, LP_WORD(L_I_ISO), &menu_scripts_iso,
                     menu_scripts_apply_calc_ev),
    MENUITEM_AV(0, LP_WORD(L_I_AV_VAL), &menu_scripts_av,
                menu_scripts_apply_calc_ev),
    MENUITEM_TIMEOUT(0, LP_WORD(L_I_TV_VAL), &menu_scripts_tv,
                     menu_scripts_apply_calc_ev),
    MENUITEM_EVINFO(0, LP_WORD(L_I_EV_VAL), &menu_scripts_ev,
                    menu_scripts_apply_calc_tv),
    MENUITEM_LAUNCH(0, LP_WORD(L_I_APPLY), menu_scripts_apply_calc),
};

menuitem_t dof_calc_items[] = {
    MENUITEM_FLENGTH(0, LP_WORD(L_I_FLENGTH), &menu_scripts_fl,
                     menu_scripts_apply_dof),
    MENUITEM_AV(0, LP_WORD(L_I_AV_VAL), &menu_DPData.av_val,
                menu_scripts_apply_dof_av),
    MENUITEM_FDIST(0, LP_WORD(L_I_FDIST), &menu_scripts_fd,
                   menu_scripts_apply_dof),
...
menuitem_t menupage_scripts_items[] = {
    MENUITEM_SUBMENU(MENUPAGE_SCRIPTS_EXTAEB, LP_WORD(L_S_EXT_AEB),
                     &ext_aeb_page, menu_scripts_ext_aeb),
    MENUITEM_SUBMENU(MENUPAGE_SCRIPTS_EFLAEB, LP_WORD(L_S_EFL_AEB),
                     &efl_aeb_page, menu_scripts_efl_aeb),
    MENUITEM_SUBMENU(MENUPAGE_SCRIPTS_APTAEB, LP_WORD(L_S_APT_AEB),
                     &apt_aeb_page, menu_scripts_apt_aeb),
    MENUITEM_SUBMENU(MENUPAGE_SCRIPTS_ISOAEB, LP_WORD(L_S_ISO_AEB),
                     &iso_aeb_page, menu_scripts_iso_aeb),
    MENUITEM_SUBMENU(MENUPAGE_SCRIPTS_INTERVAl, LP_WORD(L_S_INTERVAL),
                     &interval_page, menu_scripts_interval),
    MENUITEM_SUBMENU(MENUPAGE_SCRIPTS_BRAMP, LP_WORD(L_S_BRAMP), &bramp_page,
                     menu_scripts_bramp),
    MENUITEM_SUBMENU(MENUPAGE_SCRIPTS_HANDWAVE, LP_WORD(L_S_HANDWAVE),
                     &wave_page, menu_scripts_wave),
    MENUITEM_SUBMENU(MENUPAGE_SCRIPTS_TIMER, LP_WORD(L_S_TIMER), &timer_page,
                     menu_scripts_self_timer),
    MENUITEM_SUBMENU(MENUPAGE_SCRIPTS_LEXP, LP_WORD(L_S_LEXP), &lexp_page,
                     menu_scripts_long_exp),
    MENUITEM_SUBMENU(MENUPAGE_SCRIPTS_DOFC, LP_WORD(L_S_DOF_CALC),
                     &dof_calc_page, NULL),
};
...
void menu_lexp_calc_open(menu_t *menu) {
    ...
    menu_scripts_ev = ec_normalize(
        8.0f * (float_log2(menu_scripts_tv) + (DPData.tv_val - TV_SEC) / 8.0f));
}
...
void menu_dof_calc_open(menu_t *menu) {
    ...
    calculate_dof(menu_scripts_fl, menu_scripts_fd, menu_DPData.av_val,
                  menu_scripts_dof_min, menu_scripts_dof_max);
}
...
void menu_scripts_apply_calc_ev(const menuitem_t *item) {
    ...
    float script_ev = (menu_scripts_iso - ISO_100) / 8.0f -
                      (menu_scripts_av - AV_MIN) / 8.0f +
                      float_log2(menu_scripts_tv);
...
void menu_scripts_apply_calc_tv(const menuitem_t *item) {
    float camera_ev = (DPData.iso - ISO_100) / 8.0f -
                      ...
                      (DPData.tv_val - TV_SEC) / 8.0f;
    float script_ev =
        (menu_scripts_iso - ISO_100) / 8.0f - (menu_scripts_av - AV_MIN) / 8.0f;

    menu_scripts_tv =
        float_pow2(menu_scripts_ev / 8.0f - (camera_ev - script_ev));
