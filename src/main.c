...
void cache_hacks(void) {
    ...
    // we place our hack at the last 128kb (0x20000) of this space, at addr:
    // 0x7E0000 (see hack_relocate()) the original instruction was: MOV R1,
    // #0x800000 we change it to: MOV R1, #0x7E0000 (in binary the instruction
    // looks like: 0xE3A0187E)
    cache_fake(0xFF811318, 0xE3A0187E, TYPE_ICACHE);

    // hookup to dmProcInit(), so we can enable massive debug and run our
    // hack_pre_init_hook
    cache_fake(0xFF8111AC, ASM_BL(0xFF8111AC, &hack_dmProcInit), TYPE_ICACHE);

    // hookup our MainCtrlInit
    // cache_fake(0xFF8110E4, ASM_BL(0xFF8110E4, &hack_MainCtrlInit),
    // TYPE_ICACHE);

#ifdef ENABLE_DEBUG
    // hookup our GUI_IdleHandler
    cache_fake(0xFF82A4F0, ASM_BL(0xFF82A4F0, &hack_register_gui_idle_handler),
               TYPE_ICACHE);
#endif

    // hookup our Intercom
    cache_fake(0xFFA5D590, ASM_BL(0xFFA5D590, &hack_init_intercom_data),
               TYPE_ICACHE);
...
void hack_pre_init_hook(void) {
    ...
    cache_fake(0xFF837FEC, ASM_BL(0xFF837FEC, &hack_item_set_label),
               TYPE_ICACHE);
    cache_fake(0xFF838300, ASM_BL(0xFF838300, &hack_item_set_label),
               TYPE_ICACHE);
...
void hack_post_init_hook(void) {
    ...
    // cache_fake(0xFF92C5D8, ASM_BL(0xFF92C5D8, &hack_invert_olc_screen),
    // TYPE_ICACHE); cache_fake(0xFF92C5FC, ASM_BL(0xFF92C5FC,
    // &hack_invert_olc_screen), TYPE_ICACHE);
    ...
    // cache_fake(0xFF81B9D0, ASM_MOV_R0_INT(0), TYPE_ICACHE); // prevent ui
    // lock cache_fake(0xFF81B400, ASM_MOV_R0_INT(0), TYPE_ICACHE); // prevent
    // ui lock cache_fake(0xFF9DDB24, ASM_MOV_R0_INT(0), TYPE_ICACHE); //
    // prevent ui lock
}
