 ...
 * This means that the display shall be ON, and the sensor activated by the face. 
 * 
 * The features while looking through the viewfinder are:
 * - M mode:
 *   - SET: toggle the @ref fexp
 *   - RIGHT / LEFT: increase / decrease exposure compensation if @ref autoiso is enabled
 ...
 * 
 * When the "Use D-Pad" option is active (set to "Yes"), and if the camera is set in
 *  any creative mode, you can change the ISO value while looking through the viewfinder:
 * - Set the camera to a creative mode.
 * - Look through the viewfinder, and half-press the shutter button; you should see the 
 * current measurements (shutter speed, aperture...).
 * - Now press the LEFT and RIGHT buttons to change the ISO value, or UP just to display it; 
 *   while the button is kept pressed down, the new value will be displayed in the viewfinder, 
 *   in the same place of other information.
 * 
 ...
 * - In P or Av modes: the ISO is displayed in place of the burst counter (because speed is managed by
 *   the camera). It is coded as:
 *   | Burst counter  |  ISO value |
 *   | ---------------|-----------:|
 *   | 1              |  100       |
 *   | 2              |  200       |
 *   | 4              |  400       |
