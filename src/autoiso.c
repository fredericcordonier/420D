 ...
 * 
 * 400plus has support for AutoISO in creative modes, using our own algorithms. It works by 
 * reading the measurements that the camera makes, when you half-press the shutter button, 
 * and changing the ISO value accordingly. The behavior of AutoISO is different for each creative 
 * mode, but the idea is roughly the same: use the lowest possible ISO to maintain a suitable 
 * speed or aperture.
 * 
 * To enable AutoISO
 ...
 * 
 * Alternatively, AutoISO can be configured in the "AutoISO" sub-menu in the parameters page at 
 * @ref menu :
 * | Item    | Description |
 * |---------|------------|
 ...
 * In normal Av mode, the user selects ISO and aperture, and the camera decides the shutter speed according to the scene. 
 * But when AutoISO is active, 400plus will always try to use the lowest possible ISO value (as configured by "Min ISO"), 
 * unless the shutter speed gets too low (as configured by "Min Tv"); when that happens, 400plus will automatically raise 
 * the ISO value (up to "Max ISO") as needed.
 * 
 * Please, note that when both Safety Shift and AutoISO are enabled, the camera could drop shutter speed below the 
 * configured "Min Tv", if "Max ISO" is reached, in order to maintain a correct exposure.
 * 
 * AutoISO In Tv mode
 * In Tv mode, the intention of AutoISO is to use the lowest possible ISO that ensures an acceptable depth of field.
 * 
 * In Tv mode, AutoISO will try to keep ISO as low as possible (as configured by "Min ISO"), unless the scene requires an 
 * aperture larger than desired (as configured at "Max Av"); then, AutoISO will raise the ISO value (up to "Max ISO") as needed.
 * Please, note that the "Max Av" parameter is the difference between the maximum aperture supported by your lens and the 
 * maximum aperture that AutoISO will allow before raising the ISO value: for example, on a f/4 lens, setting "Max Av" to +1EV 
 * means that AutoISO should raise the ISO whenever the camera sets an aperture of f/5.6 or greater. On a zoom lens with a 
 * variable maximum aperture, "Max Av" is always based on the maximum aperture supported by your lens at each moment.
 * 
 * In Tv mode, it is perfectly possible to combine both Safety Shift and AutoISO at the same time: when AutoISO can no 
 * longer increase the ISO value, the camera will use a larger aperture, and when the lens' maximum aperture is reached, 
 * Safety Shift will force a longer exposure time.
 */
 ...
 * In other expert modes, change the ISO if Ev does not allow keeping the selected values of Tv and Av.
 */
void autoiso() {
	ev_t limit;
