...
void calculate_dof(int focal_length, int focus_distance, int av, char *min,
    ...
    float fn =
        float_pow2(((float)av / 8.0f - 1) /
                   2.0f); // Precise F-Number = 2^(n/2), 1/8 EV resolution
...
void enter_factory_mode() { EnterFactoryMode(); }
...
int timestamp() {
    ...
    now_ms = (long long)now_ts.tv_sec * 1000LL +
             (long long)now_ts.tv_nsec / 1000000LL;
...
// and use local buffer and local position pointer, sort of mmap'ed part of
// file. the benefit is obvious: speed the drawbacks are: 1.the routine needs to
// be informed when a new file is being parsed, so it will reset the local
// buffer this is done by calling the routine with FD == -1, it is sort of init
// call. you will have to init everytime you open a new file, before the first
// real call
