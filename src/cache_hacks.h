...
#define BL_INSTR(pc, dest)                                                     \
    (0xEB000000 | (((((uint32_t)dest) - ((uint32_t)pc) - 8) >> 2) & 0x00FFFFFF))

#define B_INSTR(pc, dest)                                                      \
    (0xEA000000 | (((((uint32_t)dest) - ((uint32_t)pc) - 8) >> 2) & 0x00FFFFFF))

#define TYPE_DCACHE 0
#define TYPE_ICACHE 1

/* get cache size depending on cache type and processor setup (13 -> 2^13 ->
 * 8192 -> 8KiB) */
...
#define CACHE_INDEX_ADDRMASK(t)                                                \
    (CACHE_INDEX_BITMASK(t) << CACHE_INDEX_TAGOFFSET(t))
...
#define CACHE_WORD_ADDRMASK(t)                                                 \
    (CACHE_WORD_BITMASK(t) << CACHE_WORD_TAGOFFSET(t))
...
#define CACHE_SEGMENT_ADDRMASK(t)                                              \
    (CACHE_SEGMENT_BITMASK(t) << CACHE_SEGMENT_TAGOFFSET(t))
...
static uint32_t cache_patch_single_word(uint32_t address, uint32_t data,
                                        uint32_t type) {
    uint32_t cache_seg_index_word =
        (address & (CACHE_INDEX_ADDRMASK(type) | CACHE_WORD_ADDRMASK(type)));
    uint32_t cache_tag_index =
        (address & (CACHE_TAG_ADDRMASK(type) | CACHE_INDEX_ADDRMASK(type))) |
        0x10;
...
/*	fetch all the instructions in that temp_cacheline the given address is
   in. this is *required* before patching a single address. if you omit this,
   only the patched instruction will be correct, the other instructions around
   will simply be crap. warning - this is doing a data fetch (LDR) so DCache
   patches may cause unwanted or wanted behavior. make sure you know what you do
   :)
...
static void cache_get_content(uint32_t segment, uint32_t index, uint32_t word,
                              uint32_t type, uint32_t *tag, uint32_t *data) {
    uint32_t cache_seg_index_word = 0;
    uint32_t stored_tag_index = 0;
    uint32_t stored_data = 0;

    cache_seg_index_word |= ((segment & CACHE_SEGMENT_BITMASK(type))
                             << CACHE_SEGMENT_TAGOFFSET(type));
    cache_seg_index_word |=
        ((index & CACHE_INDEX_BITMASK(type)) << CACHE_INDEX_TAGOFFSET(type));
    cache_seg_index_word |=
        ((word & CACHE_WORD_BITMASK(type)) << CACHE_WORD_TAGOFFSET(type));
...
static uint32_t __attribute__((unused)) cache_is_patchable(uint32_t address,
                                                           uint32_t type) {
    uint32_t stored_tag_index = 0;
    uint32_t stored_data = 0;
    cache_get_content(
        ...
        type, &stored_tag_index, &stored_data);
    ...
    /* now check if the TAG RAM content matches with what we expect and valid
     * bit is set */
    uint32_t tag_index_mask =
        CACHE_TAG_ADDRMASK(type) | CACHE_INDEX_ADDRMASK(type);
    ...
    if ((stored_tag_index & tag_index_mask) == cache_tag_index) {
        ...
        if (stored_data ==
            *(uint32_t *)address) { // data is original, so it is patchable
            return 1;
        }
...
static uint32_t cache_get_cached(uint32_t address, uint32_t type) {
    ...
    /* now check if the TAG RAM content matches with what we expect and valid
     * bit is set */
    uint32_t tag_index_valid_mask =
        CACHE_TAG_ADDRMASK(type) | CACHE_INDEX_ADDRMASK(type) | 0x10;
...
static void dcache_unlock(void) {
    ...
    for (segment = 0; segment < (1U << CACHE_SEGMENT_BITS(TYPE_DCACHE));
         segment++) {
        for (index = 0; index < (1U << CACHE_INDEX_BITS(TYPE_DCACHE));
             index++) {
            uint32_t seg_index =
                (segment << CACHE_SEGMENT_TAGOFFSET(TYPE_DCACHE)) |
                (index << CACHE_INDEX_TAGOFFSET(TYPE_DCACHE));
...
static inline void dcache_lock(void) {
    ...
    for(segment = 0; segment < (1U<<CACHE_SEGMENT_BITS(TYPE_DCACHE)); segment++
                    ...
                    uint32_t seg_index = (segment <<
    CACHE_SEGMENT_TAGOFFSET(TYPE_DCACHE)) | (index <<
    CACHE_INDEX_TAGOFFSET(TYPE_DCACHE)); asm volatile ( "MCR p15, 0, %0, c7,
    c14, 2\n"
    ...
    */
    // XXX: 0xAF: I dont know what is wrong with the code above, but it hangs
    // the camera the following function clears D Cache w/o problems
    clean_d_cache();
...
/* these are the "public" functions. please use only these if you are not sure
