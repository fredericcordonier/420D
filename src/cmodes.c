...
void cmodes_write() {
    ...
    if ((file = FIO_OpenFile(MKPATH_NEW(CMODES_CONFIG), O_CREAT | O_WRONLY)) ==
        -1)
        if (status.folder_exists ||
            (file = FIO_OpenFile(MKPATH_OLD(CMODES_CONFIG),
                                 O_CREAT | O_WRONLY)) == -1)
            goto end;
...
void cmode_recall() { cmode_recall_apply(FALSE); }

void cmode_apply() { cmode_recall_apply(TRUE); }
...
void cmode_recall_apply(int full) {
    ...
    if (AE_IS_CREATIVE(status.main_dial_ae)) {
        ...
        if (current_cmode != CMODE_NONE &&
            cmode_read(current_cmode, &snapshot)) {
            // Update current status
...
void get_cmode_filenames(char filenames[][FILENAME_LENGTH], int cmode_id) {
    sprintf(filenames[0], "%s/%s/" CMODES_FILE, FOLDER_ROOT, FOLDER_NAME,
            cmode_id);
