...
typedef enum {
    ...
    IC_SHUTDOWN = 0x52,                    //
    IC_RELEASE = 0x53,                     //
    IC_RELEASE_INHIBIT = 0x55, // forbid the shooting, if we call these, we will
                               // have to update "is_release_permitted" flag too
    IC_RELEASE_PERMIT = 0x57,  // permit the shooting
    IC_RESET_SETTINGS = 0x59,  // it is called in reset settings, then 0x5A is
                               // called. so they both should be related
    ...
    IC_HELLO_WORD =
        0x6E, // ? [First message received at intercom proxy; mostly empty]
    IC_INITIALIZATION =
        0x70,               // ? [Second message at intercom; unknown contents]
    IC_POWER_FLAG = 0x72,   //
    IC_BUZZER = 0x73,       //
    IC_TEMP = 0x74,         //
    IC_MEASURING = 0x80,    //
    ...
    IC_UNKNOWN_8D =
        0x8D, // ? [Appears after initialization, but contains no info]
...
extern int BurstCounter; // remaining shots in burst mode (displayed in VF's
                         // bottom right corner)
extern int FaceStatus;   // 0 = no face, 1 = face (disp off)... see #32, this
...
extern int Run_UnicodeString_Dataset_c(char *dest, const char *src,
                                       size_t size);

// Queue management

extern int *CreateMessageQueue(const char *nameMessageQueue, int param);
extern int ReceiveMessageQueue(void *hMessageQueue, void *pMessage,
                               int forever);
extern int PostMessageQueue(void *hMessageQueue, void *pMessage, int forever);
extern int TryPostMessageQueue(void *hMessageQueue, void *pMessage,
                               int forever);
...
// Semaphores
/*
extern SEM_ID CreateBinarySemaphore(char * name, SEM_B_STATE state); //
SEM_EMPTY (0), SEM_FULL (1) extern int TakeSemaphore(SEM_ID sem, int time);
