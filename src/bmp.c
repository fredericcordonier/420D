...
static void _draw_char(uint8_t *vram_address, unsigned fontspec,
                       uint8_t *bmp_vram_row, char c) {
    ...
    for (i = 0; i < font->height; i++) {
        ...
        for (j = 0; j < font->width / 4; j++) {
            ...
            if ((void *)row >= (void *)vram_address &&
                (void *)row < (void *)(vram_address + VramSize)) {
                *(row++) = bmp_pixels;
            } else {
...
void bmp_puts(uint8_t *vram_address, unsigned fontspec, unsigned *x,
              unsigned *y, const char *s) {
...
void bmp_printf(uint8_t *vram_address, unsigned fontspec, unsigned x,
                unsigned y, const char *fmt, ...) {
    va_list ap;
    char buf[256];
...
// Andrew:
// bmp_hexdump(FONT_SMALL, 0, y_offset, 0xlocation_to_dump, 16*[# of lines to
// show on screen]); always keep x offset as 0 because it fills the whole screen
// as is :P
//
//~ Scaled in half from ML version. always have len as a multiple of 16. General
//~ use is 16 * num of lines desired printed.
void bmp_hexdump(uint8_t *vram_address, unsigned fontspec, unsigned x,
                 unsigned y, const void *buf, int len) {
    if (len == 0)
        return;
    ...
    do {
        bmp_printf(vram_address, fontspec, x, y, "%08x: %08x %08x %08x %08x",
                   (unsigned)d, len > 0 ? MEM(d + 0) : 0,
                   len > 4 ? MEM(d + 1) : 0, len > 8 ? MEM(d + 2) : 0,
                   len > 12 ? MEM(d + 3) : 0);
