 ...
 * 
 * To access 400plus' menu, press the DP (DIRECT PRINT) button from within the main camera 
 * dialog; to exit 400plus' menu, just half-press the shutter-release button. The DP
 * button is located on the rear of the camera, at the left of the viewfinder, and beside the 
 * blue printer icon with a squiggly arrow line next to it.
 * 
 * <H1>Navigating The Menu</H1>
 * 
 * 400plus features a multi-page menu, where each page is dedicated to a certain task. To navigate 
 * the pages, use the front wheel (notice the '<<' and '>>' indicators at the header of the menu).
 * There is also a method to quickly access any page: press and hold the AV button down, then a list 
 * of all available pages will appear; now use the front dial to select a page, and release the AV 
 * button to jump into it. Moreover, if the "Navigate to main" option is active (see Settings Page),
 *  you can also press and immediately release the AV button to stay in the main page; once there, 
 * just use UP (ISO) and DOWN (WB) to select a page, and SET to enter it.
 * To navigate within a page, use the UP and DOWN buttons, (or ZOOM IN and ZOOM OUT, to move faster);
 *  then use the LEFT (METERING) and RIGHT (AF) buttons to change the value of the selected item 
 * (hold these buttons down to change the value faster). Items with an exclamation mark '!' at the 
 * left perform an action: activate it with the SET button. And Items with a right sign '>' are 
 * sub-menus: enter them with the RIGHT button, and return back to the main page with the AV
 * button. Notice that some items can both perform an action and have a sub-menu.
 * 
 * <H1>Menu pages</H1>
 * <H2>Parameters Page</H2>
 * 
 *  This page contains extensions to the camera's shooting parameters, and also provides a quick 
 * access to some commonly-used, but hard to reach, custom functions.
 * 
 * | Item                |  Description                                |
 * |---------------------|---------------------------------------------|
 ...
 * 
 * <H2>Settings Page</H2>
 * Some advanced settings about your camera and 400plus; please, see Configuration, page 48 for more information.
 * | Item                       | Description                         |
 * |----------------------------|-------------------------------------|
 ...
 * While in any page within the menu, you have the ability to change the order of the items to your own personal 
 * preference. 
 * 
 * To do this, just press the PLAY button to 'grab' an item (a small '>' will be displayed at the 
 * left of the item). 
 * 
 * Now use the UP and DOWN buttons to position the item where you would like it to reside 
 * (you will see the item actually move to the new position). Finish the process by pressing the PLAY button again, 
