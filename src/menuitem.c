...
void menuitem_print(char *buffer, const char *name, const char *parameter,
                    const int length);
...
void menuitem_display_ec(const menuitem_t *item, char *buffer,
                         const int length) {
    char value[LP_MAX_WORD];

    if (item->parm.menuitem_ec.zero_means_off &&
        *item->parm.menuitem_ec.value == 0) {
...
void menuitem_display_av(const menuitem_t *item, char *buffer,
                         const int length) {
    char value[LP_MAX_WORD];
...
void menuitem_display_tv(const menuitem_t *item, char *buffer,
                         const int length) {
    char value[LP_MAX_WORD];
...
void menuitem_display_bulb(const menuitem_t *item, char *buffer,
                           const int length) {
    char value[LP_MAX_WORD];
...
void menuitem_display_iso(const menuitem_t *item, char *buffer,
                          const int length) {
    char value[LP_MAX_WORD];
...
void menuitem_display_int(const menuitem_t *item, char *buffer,
                          const int length) {
    char value[LP_MAX_WORD];

    if (item->parm.menuitem_int.zero_means_unlimited &&
        *item->parm.menuitem_int.value == 0) {
        menuitem_print(buffer, item->name, LP_WORD(L_V_NO_LIMIT), length);
    } else {
        sprintf(value, item->parm.menuitem_int.format,
                *item->parm.menuitem_int.value);
        menuitem_print(buffer, item->name, value, length);
    }
}

void menuitem_display_time(const menuitem_t *item, char *buffer,
                           const int length) {
    char value[LP_MAX_WORD];
...
void menuitem_display_enum(const menuitem_t *item, char *buffer,
                           const int length) {
    if (*item->parm.menuitem_enum.value >= 0 &&
        *item->parm.menuitem_enum.value < item->parm.menuitem_enum.list->size)
        menuitem_print(buffer, item->name,
                       item->parm.menuitem_enum.list
                           ->data[*item->parm.menuitem_enum.value],
                       length);
    else
        menuitem_print(buffer, item->name, "?", length);
}

void menuitem_display_info(const menuitem_t *item, char *buffer,
                           const int length) {
    menuitem_print(
        buffer, item->name,
        item->parm.menuitem_info.value ? item->parm.menuitem_info.value : "",
        length);
}

void menuitem_display_sub(const menuitem_t *item, char *buffer,
                          const int length) {
    menuitem_print(buffer, item->name, ">", length);
}
...
void menuitem_print(char *buffer, const char *name, const char *parameter,
    ...
    if (pad > 0)
        ...
        sprintf(buffer, "%s %.*s.", name, strlen_utf8(parameter) + pad - 1,
                parameter);
}

void menuitem_inc_ec(const menuitem_t *item, const int repeating) {
    *item->parm.menuitem_ec.value =
        ec_inc(*item->parm.menuitem_ec.value, item->parm.menuitem_ec.extended);
}
...
void menuitem_inc_iso(const menuitem_t *item, const int repeating) {
    if (repeating || item->parm.menuitem_iso.full)
        *item->parm.menuitem_iso.value =
            MAX(*item->parm.menuitem_iso.value,
                iso_next(*item->parm.menuitem_iso.value));
    else
        *item->parm.menuitem_iso.value =
            iso_inc(*item->parm.menuitem_iso.value);
}
...
void menuitem_inc_int(const menuitem_t *item, const int repeating) {
    ...
    if (item->parm.menuitem_int.base_log) {
        while (*item->parm.menuitem_int.value /
                   item->parm.menuitem_int.base_log >=
               big_step) {
    ...
    *item->parm.menuitem_int.value += step;
    ...
    if (item->parm.menuitem_int.base_log)
        *item->parm.menuitem_int.value =
            step * (*item->parm.menuitem_int.value / step);

    *item->parm.menuitem_int.value =
        MIN(*item->parm.menuitem_int.value, item->parm.menuitem_int.max);
}
...
void menuitem_inc_enum(const menuitem_t *item, const int repeating) {
    if (*item->parm.menuitem_enum.value >=
        item->parm.menuitem_enum.list->size - 1) {
...
void menuitem_dec_ec(const menuitem_t *item, const int repeating) {
    if (item->parm.menuitem_ec.zero_means_off &&
        *item->parm.menuitem_ec.value < 0x05)
        *item->parm.menuitem_ec.value =
            item->parm.menuitem_ec.can_do_zero ? 0x00 : EV_STEP;
    else
        *item->parm.menuitem_ec.value = ec_dec(*item->parm.menuitem_ec.value,
                                               item->parm.menuitem_ec.extended);
}
...
void menuitem_dec_iso(const menuitem_t *item, const int repeating) {
    if (repeating || item->parm.menuitem_iso.full)
        *item->parm.menuitem_iso.value =
            iso_prev(*item->parm.menuitem_iso.value);
    else
        *item->parm.menuitem_iso.value =
            iso_dec(*item->parm.menuitem_iso.value);
}
...
void menuitem_dec_int(const menuitem_t *item, const int repeating) {
    ...
    *item->parm.menuitem_int.value =
        MAX(*item->parm.menuitem_int.value, item->parm.menuitem_int.min);
}
