...
int lang_pack_loader(void *user, int lineno, const char *section,
                     const char *name, const char *value);

// put the language keys into the hack, we need them to match the keys in
// languages.ini file
...
// store English language into the hack, so we have at least one language
// available.
...
void lang_pack_init() {
    ...
    strncpy0(languages_found[languages_found_last++], "Camera",
             LP_MAX_WORD - 1);
    strncpy0(languages_found[languages_found_last++], "ENGLISH",
             LP_MAX_WORD - 1);
    languages_found[languages_found_last][0] = '\0';

    if ((res = ini_parse(MKPATH_NEW(LANGUAGES_FILENAME), NULL, NULL,
                         lang_pack_sections, NULL)) == -1)
        res = ini_parse(MKPATH_OLD(LANGUAGES_FILENAME), NULL, NULL,
                        lang_pack_sections, NULL);
...
int lang_pack_loader(void *user, int lineno, const char *section,
    ...
    for (i = L_FIRST; i < L_COUNT; i++) {
        if (!strncmp(lang_pack_keys[i], name, LP_MAX_WORD - 1)) {
            ...
            // debug_log("LANG: setting key [%s]: [%s]", lang_pack_keys[i],
            // lang_pack_current[i]);
...
void lang_pack_config() {
    ...
    debug_log("Setting language to: [%s] <%s>", lang,
              (settings.language ? "forced" : "camera"));
    ...
    for (i = L_FIRST; i < L_COUNT; i++) {
        if (lang_pack_english[i] != NULL) {
            ...
            // debug_log("LANG: setting ENG key [%d]: [%s]", i,
            // lang_pack_current[i]);
        } else {
    ...
    if (settings.language != 0 || DPData.language > 0 /* ENGLISH */) {
        ...
        if ((res = ini_parse(MKPATH_NEW(LANGUAGES_FILENAME), lang,
                             lang_pack_loader, NULL, (void *)lang)) == -1)
            res = ini_parse(MKPATH_OLD(LANGUAGES_FILENAME), lang,
                            lang_pack_loader, NULL, (void *)lang);

        if (res == 0) {
            debug_log("[%d] keys loaded from languages.ini.",
                      lang_pack_keys_loaded);
        } else {
