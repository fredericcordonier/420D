...
int ini_parse_file(int file, const char *wanted_section,
    ...
    while (hack_fgets(line, sizeof(line), file) != NULL) {
        ...
        if (*prev_name && *start && start > line) {
            ...
            if (section_found && handler &&
                !handler(user, lineno, section, prev_name, start) && !error)
                error = lineno;
        } else
#endif
            if (*start == ';' || *start == '#') {
            ...
            if (*end == '=' || *end == ':') {
                *end = '\0';
                ...
                if (section_found && handler &&
                    !handler(user, lineno, section, name, value) && !error)
                    error = lineno;
            } else if (!error) {
