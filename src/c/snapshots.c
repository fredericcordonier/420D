...
int snapshot_write(char names[][FILENAME_LENGTH]) {
    ...
    if ((file = FIO_OpenFile(names[0], O_CREAT | O_WRONLY)) == -1)
        if (status.folder_exists ||
            (file = FIO_OpenFile(names[1], O_CREAT | O_WRONLY)) == -1)
            goto end;

    if (FIO_WriteFile(file, (void *)&version, sizeof(version)) !=
        sizeof(version))
        goto end;
...
void snapshot_apply(snapshot_t *snapshot) {
    ...
    if (cmodes_config.recall_settings) {
        send_to_intercom(IC_SET_AUTO_POWER_OFF,
                         snapshot->DPData.auto_power_off);
        ...
        send_to_intercom(IC_SET_LCD_BRIGHTNESS,
                         snapshot->DPData.lcd_brightness);
        send_to_intercom(IC_SET_DATE_TIME, snapshot->DPData.date_time);
        send_to_intercom(IC_SET_FILE_NUMBERING,
                         snapshot->DPData.file_numbering);
    ...
    if (cmodes_config.recall_cfn) {
        send_to_intercom(IC_SET_CF_SET_BUTTON_FUNC,
                         snapshot->DPData.cf_set_button_func);
        send_to_intercom(IC_SET_CF_NR_FOR_LONG_EXPOSURE,
                         snapshot->DPData.cf_nr_for_long_exposure);
        send_to_intercom(IC_SET_CF_EFAV_FIX_X, snapshot->DPData.cf_efav_fix_x);
        send_to_intercom(IC_SET_CF_AFAEL_ACTIVE_BUTTON,
                         snapshot->DPData.cf_afael_active_button);
        send_to_intercom(IC_SET_CF_EMIT_AUX, snapshot->DPData.cf_emit_aux);
        send_to_intercom(IC_SET_CF_EXPLEVEL_INC_THIRD,
                         snapshot->DPData.cf_explevel_inc_third);
        ...
        send_to_intercom(IC_SET_CF_AEB_SEQUENCE,
                         snapshot->DPData.cf_aeb_sequence);
        send_to_intercom(IC_SET_CF_SI_INDICATE,
                         snapshot->DPData.cf_si_indicate);
        send_to_intercom(IC_SET_CF_MENU_POS, snapshot->DPData.cf_menu_pos);
        send_to_intercom(IC_SET_CF_MIRROR_UP_LOCK,
                         snapshot->DPData.cf_mirror_up_lock);
        send_to_intercom(IC_SET_CF_FPSEL_METHOD,
                         snapshot->DPData.cf_fpsel_method);
        send_to_intercom(IC_SET_CF_FLASH_METERING,
                         snapshot->DPData.cf_flash_metering);
        send_to_intercom(IC_SET_CF_FLASH_SYNC_REAR,
                         snapshot->DPData.cf_flash_sync_rear);
        send_to_intercom(IC_SET_CF_SAFETY_SHIFT,
                         snapshot->DPData.cf_safety_shift);
        send_to_intercom(IC_SET_CF_LENS_BUTTON,
                         snapshot->DPData.cf_lens_button);
        send_to_intercom(IC_SET_CF_ORIGINAL_EVAL,
                         snapshot->DPData.cf_original_eval);
        send_to_intercom(IC_SET_CF_QR_MAGNIFY, snapshot->DPData.cf_qr_magnify);
        send_to_intercom(IC_SET_CF_TFT_ON_POWER_ON,
                         snapshot->DPData.cf_tft_on_power_on);
    }
