...
menuitem_t scripts_items[] = {
    MENUITEM_BOOLEAN(0, LP_WORD(L_I_KEEP_POWER_ON), &settings.keep_power_on,
                     NULL),
    ...
    MENUITEM_SCRIND(0, LP_WORD(L_I_INDICATOR), &settings.script_indicator,
                    NULL),
};
...
menuitem_t cmodes_items[] = {
    MENUITEM_BOOLEAN(0, LP_WORD(L_I_CMODES_CAMERA),
                     &cmodes_config.recall_camera, NULL),
    MENUITEM_BOOLEAN(0, LP_WORD(L_I_CMODES_420D), &cmodes_config.recall_420D,
                     NULL),
    MENUITEM_BOOLEAN(0, LP_WORD(L_I_CMODES_ORDERING),
                     &cmodes_config.recall_ordering, NULL),
    MENUITEM_BOOLEAN(0, LP_WORD(L_I_CMODES_SETTINGS),
                     &cmodes_config.recall_settings, NULL),
    MENUITEM_BOOLEAN(0, LP_WORD(L_I_CMODES_IMAGE), &cmodes_config.recall_image,
                     NULL),
    MENUITEM_BOOLEAN(0, LP_WORD(L_I_CMODES_CFN), &cmodes_config.recall_cfn,
                     NULL),
};

menuitem_t menus_items[] = {
    MENUITEM_BOOLEAN(0, LP_WORD(L_I_WRAP_MENUS), &settings.menu_wrap, NULL),
    MENUITEM_BOOLEAN(0, LP_WORD(L_I_NAVIGATE_MAIN), &settings.menu_navmain,
                     NULL),
    MENUITEM_BOOLEAN(0, LP_WORD(L_I_ENTER_MAIN), &settings.menu_entermain,
                     NULL),
...
menuitem_t menu_settings_items[] = {
    MENUITEM_LANG(MENUPAGE_SETTINGS_LANGUAGE, LP_WORD(L_I_LANGUAGE),
                  &settings.language, reload_language_and_refresh),
    MENUITEM_DIG_ISO(MENUPAGE_SETTINGS_ISOSTEP, LP_WORD(L_I_DIG_ISO_STEP),
    ...
    MENUITEM_SUBMENU(MENUPAGE_SETTINGS_SCRIPTS, LP_WORD(L_S_SCRIPTS),
                     &scripts_page, NULL),
    MENUITEM_SUBMENU(MENUPAGE_SETTINGS_BUTTONS, LP_WORD(L_S_BUTTONS),
                     &buttons_page, NULL),
    MENUITEM_SUBMENU(MENUPAGE_SETTINGS_CMODES, LP_WORD(L_S_CMODES),
                     &cmodes_page, NULL),
    MENUITEM_SUBMENU(MENUPAGE_SETTINGS_MENUS, LP_WORD(L_S_MENUS), &menus_page,
                     NULL),
    MENUITEM_SUBMENU(MENUPAGE_SETTINGS_QEXP, LP_WORD(L_S_QEXP), &qexp_page,
                     NULL),
    MENUITEM_SUBMENU(MENUPAGE_SETTINGS_PAGES, LP_WORD(L_S_PAGES), &pages_page,
                     NULL),
    MENUITEM_SUBMENU(MENUPAGE_SETTINGS_RESTORE, LP_WORD(L_I_RESTORE),
                     &restore_page, NULL),
    MENUITEM_BOOLEAN(MENUPAGE_SETTINGS_DEVEL, LP_WORD(L_I_DEVELOPERS_MENU),
                     &settings.developers_menu, NULL),
};
...
void menu_settings_open(menu_t *menu) {
    ...
    for (i = 0; i < MAX_LANGUAGES && languages_found[i] != NULL &&
                languages_found[i][0] != '\0';
         i++) {
        menupage_settings.items.data[0].parm.menuitem_enum.list->size = i + 1;
        menupage_settings.items.data[0].parm.menuitem_enum.list->data[i] =
            languages_found[i];
    }
}
