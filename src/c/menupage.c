...
void menupage_up(menu_t *menu) {
    ...
    if ((page->items.size > MENU_HEIGHT && settings.menu_wrap) ||
        ...
        if (item_grabbed) {
            SWAP(page->ordering[get_item_id(page, page->current_posn)],
                 page->ordering[get_item_id(page, page->current_posn + 1)]);
            display = TRUE;
        }
    }
...
void menupage_down(menu_t *menu) {
    ...
    if ((page->items.size > MENU_HEIGHT && settings.menu_wrap) ||
        page->current_posn < page->items.size - 1) {
        page->current_posn++;
...
void menupage_display_line(menupage_t *page, const int display_line) {
    ...
    if (item) {
        if (page->ordering && item_grabbed &&
            get_item_id(page, item_id) == get_item_id(page, page->current_posn))
            message[i++] = '>';
        else if (page->highlight &&
                 page->highlighted_item == get_real_id(page, item_id))
            message[i++] = '*';
        else
            message[i++] = ' ';
