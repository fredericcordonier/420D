...
reaction_t reaction_main_dp = {TRUE, menu_main_start},
...
reaction_t *button_actions_main[BUTTON_COUNT] = {
    [BUTTON_DP] = &reaction_main_dp,     [BUTTON_DISP] = &reaction_main_disp,
    [BUTTON_JUMP] = &reaction_main_jump, [BUTTON_TRASH] = &reaction_main_trash,
    [BUTTON_AV] = &reaction_main_av,     [BUTTON_UP] = &reaction_main_up,
    [BUTTON_DOWN] = &reaction_main_down, [BUTTON_LEFT] = &reaction_main_left,
};

reaction_t reaction_420D_dp = {TRUE, menu_event_dp},
...
reaction_t reaction_shortcut_disp = {TRUE, shortcut_event_disp},
...
/**
 * @brief Actions during "shortcut" (while pressing a shortcut
 * button=Trash/Jump)
 *
 */
reaction_t *button_actions_shortcut[BUTTON_COUNT] = {
    ...
    [BUTTON_RELEASE] =
        &reaction_shortcut_release, ///< BUTTON_RELEASE is a pseudo button
                                    ///< (release of any button)
};

/** Link DP button (blocking) to set Spot metering */
reaction_t reaction_meter_dp = {TRUE, set_metering_spot};
...
/** Management of custom color temperatures.
 *
 * Pressing DP while setting White balance opens a menu presenting the custom
 * temperatures read in the file NAMED_TEMPS.CSV in the sub-folder 420D. Anyone
 * can define his own preferred custom color temperatures by editing this file.
 ...
 * - pressing the <SET> button stores the selected value as white balance
 */
reaction_t reaction_wb_dp = {TRUE, menu_named_temps_start};
...
reaction_t reaction_face_set = {TRUE, viewfinder_set},
           /** The following reactions include a "button up" action to restore
              data display in WF */
...
reaction_t *button_actions_face[BUTTON_COUNT] = {
    [BUTTON_SET] = &reaction_face_set,   [BUTTON_UP] = &reaction_face_up,
    [BUTTON_DOWN] = &reaction_face_down, [BUTTON_RIGHT] = &reaction_face_right,
 ...
 */
reaction_t reaction_af_set = {TRUE, afp_center},
...
reaction_t *button_actions_af[BUTTON_COUNT] = {
    [BUTTON_SET] = &reaction_af_set,   [BUTTON_UP] = &reaction_af_up,
    [BUTTON_DOWN] = &reaction_af_down, [BUTTON_RIGHT] = &reaction_af_right,
    [BUTTON_LEFT] = &reaction_af_left, [BUTTON_DISP] = &reaction_af_disp,
};
 ...
 *
 */
chain_t chain_actions_main = {button_actions_main},
...
int can_hold[BUTTON_COUNT] = {
    [BUTTON_AV] = TRUE,    [BUTTON_UP] = TRUE,   [BUTTON_DOWN] = TRUE,
    [BUTTON_RIGHT] = TRUE, [BUTTON_LEFT] = TRUE,
};
 ...
 * 		- GUIMODE_SHORTCUT: while we are pressing a button assigned to a
 * shortcut (Jump/Trash)
 ...
 * 		- chain is linked to the action on button (reaction) (struct
 * reaction_t)
 * 3. Analyse reaction (action_pressed) and launch action if it exists
 * 4. Check if the button can be hold for an action (can_hold). If it is the
 * case:
 * 		- status.button_down will keep the reference of the button being
 * hold
 * 		- button_up_action and button_up_block will keep te data linked
 * to this button
...
int button_handler(button_t button, int is_button_down) {
    static action_t button_up_action =
        NULL; // Action that must be executed when the current button is
              // released
    static int button_up_block =
        FALSE; // Reaction when the current button is released
    ...
    if (is_button_down) {
        ...
        if ((chain = button_chains[gui_mode]) == NULL) {
            ...
            if ((reaction = chain->reaction[button]) == NULL) {
                debug_log("gui_mode[0x%X]: btn[0x%X] no action", gui_mode,
                          button);
                return FALSE;
            } else {
