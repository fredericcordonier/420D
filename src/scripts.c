...
void script_interval() {
    ...
    for (i = 0; i < settings.interval_shots || settings.interval_shots == 0;
         i++) {
...
void script_bramp() {
    ...
    if (settings.bramp_ramp_s > 0) {
        coef_s_expo = ((float)settings.bramp_ramp_exp / 8.0f) /
                      (float)settings.bramp_ramp_s;
        coef_s_delay = ((float)settings.bramp_ramp_time / 8.0f) /
                       (float)settings.bramp_ramp_s;
    } else {
        coef_s_expo = 0.0f;
        coef_s_delay = 0.0f;
    ...
    if (settings.bramp_ramp_t > 0) {
        coef_t_expo = ((float)settings.bramp_ramp_exp / 8.0f) /
                      ((float)settings.bramp_ramp_t * (float)TIME_RESOLUTION);
        coef_t_delay = ((float)settings.bramp_ramp_time / 8.0f) /
                       ((float)settings.bramp_ramp_t * (float)TIME_RESOLUTION);
    } else {
        coef_t_expo = 0.0f;
        coef_t_delay = 0.0f;
    ...
    for (shot = 0; shot < settings.bramp_shots || settings.bramp_shots == 0;
         shot++) {
...
void script_start() {
    ...
    if (settings.script_indicator != SCRIPT_INDICATOR_NONE) {
        if (feedback_task == NULL)
            feedback_task =
                CreateTask("Feedback", 5, 0x2000, script_feedback, 0);
        else
...
void action_ext_aeb() {
    if (DPData.tv_val == TV_VAL_BULB) {
        ...
        for (tv_val = settings.eaeb_tv_max; tv_val <= settings.eaeb_tv_min;
             tv_val = bulb_next(tv_val)) {
            wait_for_camera();
        ...
        while (frames > 0) {
            if (frames > 0 &&
                (settings.eaeb_direction == EAEB_DIRECTION_BOTH ||
                 settings.eaeb_direction == EAEB_DIRECTION_DOWN)) {
                wait_for_camera();
            ...
            if (frames > 0 && (settings.eaeb_direction == EAEB_DIRECTION_BOTH ||
                               settings.eaeb_direction == EAEB_DIRECTION_UP)) {
                wait_for_camera();
...
void action_efl_aeb() {
    ...
    while (frames > 0) {
        if (frames > 0 && (settings.efl_aeb_direction == EAEB_DIRECTION_BOTH ||
                           settings.efl_aeb_direction == EAEB_DIRECTION_DOWN)) {
            wait_for_camera();
        ...
        if (frames > 0 && (settings.efl_aeb_direction == EAEB_DIRECTION_BOTH ||
                           settings.efl_aeb_direction == EAEB_DIRECTION_UP)) {
            wait_for_camera();
...
void action_apt_aeb() {
    ...
    // ...and do the rest ourselves
    while (frames > 0) {
        if (frames > 0 && (settings.apt_aeb_direction == EAEB_DIRECTION_BOTH ||
                           settings.apt_aeb_direction == EAEB_DIRECTION_DOWN)) {
            wait_for_camera();
        ...
        if (frames > 0 && (settings.apt_aeb_direction == EAEB_DIRECTION_BOTH ||
                           settings.apt_aeb_direction == EAEB_DIRECTION_UP)) {
            wait_for_camera();
