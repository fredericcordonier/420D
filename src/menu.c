...
static int menu_event_handler(dialog_t *dialog, int *r1, gui_event_t event,
                              int *r3, int r4, int r5, int r6, int code);
...
void menu_return(menu_t *menu) { menu_set_page(get_selected_page()); }
...
void menu_right(menu_t *menu) { menu_repeat(menu, menu_repeat_right); }

void menu_left(menu_t *menu) { menu_repeat(menu, menu_repeat_left); }
...
static menupage_t *get_selected_page() {
    if (current_menu->ordering)
        return current_menu->pages
            .data[current_menu->ordering[current_menu->current_posn]];
    else
