...
#define FONT(font, fg, bg)                                                     \
    (0 | ((font) & FONT_MASK) | ((bg) & 0xFF) << 8 | ((fg) & 0xFF) << 0)
...
// max x: 45
// max y: 20
#define lcd_printf(x, y, f...)                                                 \
    bmp_printf(VramAddress, FONT(FONT_SMALL, COLOR_BLACK, COLOR_GRAY), x * 8,  \
               y * 12, ##f);
extern void bmp_printf(uint8_t *vram_address, unsigned fontspec, unsigned x,
                       unsigned y, const char *fmt, ...)
    __attribute__((format(printf, 5, 6)));
extern void bmp_hexdump(uint8_t *vram_address, unsigned fontspec, unsigned x,
                        unsigned y, const void *buf, int len);
extern void bmp_puts(uint8_t *vram_address, unsigned fontspec, unsigned *x,
                     unsigned *y, const char *s);

#define bmp_printf_timed(time, font, x, y, f...)                               \
    do {                                                                       \
