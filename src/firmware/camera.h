 ...
 * @file camera.h
 *
 * @brief Contains definitions related to camera parameters (from camera
 * memomry)
 *
 *
 ...
 *   zzz = Int. ISO: 000 => BaseISO, 001 => BaseISO + 1/8EV, 010 => BaseISO +
 * 2/8EV, ...
 *
 * [K] EV encoding
 ...
 *    yyy = Int. value: 000 => BaseEV, 011 => BaseEV + 1/3EV, 100 => BaseEV +
 * 1/2EV, 101 => BaseEv + 2/3EV [*]
 *   [*]: Could this be like [J], so 011 => BaseEV + 3/8EV , 100 => BaseEV +
 * 4/8EV, 101 => BaseEv + 5/8EV? Could we have eight intermediate EV's, instead
 * of three?
 *
 */
...
/**
 * @brief Investigations about a structure that would have the same size as
 * DPData
 *
 */
...
typedef enum { METERING_MODE_EVAL = 0, METERING_MODE_SPOT = 3 } METERING_MODE;
...
typedef enum { REVIEW_OFF = 0x00, REVIEW_HOLD = 0xFF } REVIEW_TIME;
...
typedef enum { VIDEO_SYSYEM_PAL = 0x00, VIDEO_SYSTEM_NTSC = 0x01 } VIDEO_SYSTEM;
...
typedef enum { COLOR_SPACE_SRGB = 0x00, COLOR_SPACE_ADOBE = 0x01 } COLOR_SPACE;
...
typedef enum { IMG_FORMAT_JPG = 0x01, IMG_FORMAT_RAW = 0x02 } IMG_FORMAT;

// [H] Values for "img_quality"
typedef enum { IMG_QUALITY_HIGH = 0x00, IMG_QUALITY_LOW = 0x01 } IMG_QUALITY;
...
/**
 * @brief checks the "is_release_permitted" and "BurstCounter", return 1 if we
 * can shoot
 *
