...
const int mem_spy_addresses[] = {
    0xC0220130, 0xC0220134}; // 0xc0000044, 0xc0000048, 0xc0000057, 0xc00011cf,
                             // 0xc02000a8, 0xc02000ac, };
int mem_spy_len =
    0x40 / 4; // look at ### int32's; use only when mem_spy_fixed_addresses = 0
// int mem_spy_len =
// ((int)(sizeof(mem_spy_addresses)/sizeof((mem_spy_addresses)[0]))); // use
// this when mem_spy_fixed_addresses = 1
...
// int mem_spy_value_hi = 50;  // or look for a specific range of values (0 =
// disable)
int mem_spy_value_hi =
    0; // or look for a specific range of values (0 = disable)
...
static void dbg_memspy_init() { // initial state of the analyzed memory
    bmp_printf(FONT_MED, 10, 10, "memspy init @ f:%x ... (l:+%x) ... t:%x",
               mem_spy_start, mem_spy_len, mem_spy_start + mem_spy_len * 4);
    SleepTask(1000);

    // debug_log("memspy init @ from:%x ... (len:%x) ... to:%x", mem_spy_start,
    // mem_spy_len, mem_spy_start + mem_spy_len * 4); mem_spy_len is number of
    // int32's
    if (!dbg_memmirror)
        dbg_memmirror = AllocateMemory(mem_spy_len * 4 +
                                       100); // local copy of mem area analyzed
    if (!dbg_memmirror)
        return;
    if (!dbg_memchanges)
        dbg_memchanges = AllocateMemory(mem_spy_len * 4 +
                                        100); // local copy of mem area analyzed
    if (!dbg_memchanges)
        return;
    // if (!dbg_memlogged) dbg_memlogged = AllocateMemory(mem_spy_len*4 + 100);
    // // is this change logged if (!dbg_memlogged) return;
    int i;
    int crc = 0;
...
static void dbg_memspy_update() {
    ...
    for (i = 0; i < mem_spy_len; i++) {
        ...
        if (oldval != newval) {
            ...
            if (dbg_memchanges[i] < 1000000)
                dbg_memchanges[i]++;
            // dbg_memlogged[i]=0;
            fnt = FONT(FONT_SMALL, 5, COLOR_BG);
            if (elapsed_time < mem_spy_start_time)
                dbg_memchanges[i] = 1000000; // so it will be ignored
        }

        if (mem_spy_bool && newval != 0 && newval != 1 && newval != -1)
            continue;

        if (mem_spy_value_lo && newval < mem_spy_value_lo)
            continue;
        if (mem_spy_value_hi && newval > mem_spy_value_hi)
            continue;

        if (mem_spy_count_lo && dbg_memchanges[i] < mem_spy_count_lo)
            continue;
        if (mem_spy_count_hi && dbg_memchanges[i] > mem_spy_count_hi)
            continue;

        int freq = dbg_memchanges[i] / elapsed_time;
        if (mem_spy_freq_lo && freq < mem_spy_freq_lo)
            continue;
        if (mem_spy_freq_hi && freq > mem_spy_freq_hi)
            continue;
